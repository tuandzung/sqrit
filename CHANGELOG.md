...
- Clipboard copies (`yc` / `yy` / `ya` in Results) no longer flicker into oblivion on Linux/X11. The previous implementation constructed `arboard::Clipboard` inside the copy function and dropped it on return, killing X11's selection-serve thread before the user's clipboard manager could sample the new contents — arboard logged `"Clipboard was dropped very quickly after writing (0ms); clipboard managers may not have seen the contents."`. The new `crate::clipboard::ClipboardWriter` holds one `arboard::Clipboard` for the app's lifetime (lazy-initialised on first copy, latched on init failure) so the serve thread persists between copies. A regression test in `tests/clipboard_writer.rs` locks the "one handle per lifetime" invariant by asserting `init_attempts()` stays ≤ 1 across N copies. The free `clipboard::copy_to_clipboard` is deprecated; call sites now use `app.clipboard_writer.copy(...)`.
