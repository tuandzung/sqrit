...
**Async result flow**: `AsyncResult` enum carries `QueryDone`, `SchemaLoaded`, `Connected`, and `ConnectFailed` messages through the channel. `drain_async_results()` processes them each tick. `query_id` counter prevents stale result overwrites.
