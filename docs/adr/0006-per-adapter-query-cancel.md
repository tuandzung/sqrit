...
- **Transaction state after cancel is backend-specific and not normalised.** Each engine behaves differently and sqrit does not paper over the difference; the status bar surfaces what the backend reports.
  - **PostgreSQL**: cancelling a statement inside a transaction (`BEGIN ... statement ...`) puts the transaction into the `aborted` state. Every subsequent statement on that connection returns `current transaction is aborted, commands ignored until end of transaction block` until a `ROLLBACK` is issued. The transaction itself is **not** auto-rolled-back by the cancel; the user must send `ROLLBACK` (or close the connection) to recover.
  - **MySQL**: `KILL QUERY` terminates the running statement only; the session remains open and the surrounding transaction stays open with whatever effects already committed inside it (InnoDB row-level locks held by the cancelled statement are released, but earlier statements in the transaction are not rolled back). The user must `COMMIT` or `ROLLBACK` explicitly.
  - **SQLite**: `sqlite3_interrupt` rolls back the currently-executing statement only. If the cancelled statement was inside `BEGIN ... COMMIT`, the transaction stays open with the prior statements intact, and the user must `COMMIT` or `ROLLBACK`.
  - For v0.2, sqrit does not auto-issue a `ROLLBACK` after cancel — that would discard intentional prior work in the same transaction. The status-bar message includes a hint (e.g. "query cancelled — transaction may need ROLLBACK") when the cancelled query was part of an open transaction. Auto-rollback or session-reset is deferred to v0.3+ pending real usage feedback.
